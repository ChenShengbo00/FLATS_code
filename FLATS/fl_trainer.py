...
def estimate_wg(model, device, train_loader, optimizer, epoch, log_interval, criterion):
    ...
    if batch_idx % log_interval == 0:
        logger.info('Prox-attack  Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if not pgd_attack:
            ...
            if proj == "l_inf":
                ...
                # do l2_projection if proj == "l_inf":
                adv_optimizer.step()
                w = list(model.parameters())
...
def test(model, device, test_loader, test_batch_size, criterion, mode="raw-task", dataset="cifar10", poison_type="fashion"):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            print('test : output,target', output.size(), target.size(), type(target), type(output))
...
class FrequencyFederatedLearningTrainer(FederatedLearningTrainer):
    ...
    def run(self):
        # poisoned_train_loader, vanilla_test_loader, targetted_task_test_loader, num_dps_poisoned_dataset, clean_train_loader
        print('****************************************FL_trainer:run()**********************************')
        main_task_acc = []
        raw_task_acc = []
        backdoor_task_acc = []
        fl_iter_list = []
        ...
        for flr in range(1, self.fl_round+1):
            print("########################fl rounds: {}".format(self.attacking_fl_rounds))
            g_user_indices = []
            ...
            if flr in self.attacking_fl_rounds: #攻击的轮数
                ...
                print("FL round: {}, total num data points: {}, num dps poisoned: {}".format(flr, num_data_points, self.num_dps_poisoned_dataset))
                ...
                print("Net freq: {}, FL round: {} with adversary".format(net_freq, flr))
                #pdb.set_trace()
                # we need to reconstruct the net list at the beginning 重建参与者的网络
                ...
                for net_idx, net in enumerate(net_list):
                    ...
                    if net_idx == 0: # 攻击
                        print('攻击！')
                        print(self.poisoned_emnist_train_loader)
                        if self.prox_attack:
                            # estimate w_hat
                        ...
                        test(net, self.device, self.targetted_task_test_loader, test_batch_size=self.test_batch_size,
                             criterion=self.criterion, mode="targetted-task", dataset=self.dataset, poison_type=self.poison_type)
            ...
            # if flr in self.attacking_fl_rounds, else
            else: #不在攻击范围内
                # 选择用户，选出数据
            ...
            print('========================================================after local training periods FEDAVG\n')
            ...
            overall_acc, raw_acc = test(self.net_avg, self.device, self.vanilla_emnist_test_loader, test_batch_size=self.test_batch_size, criterion=self.criterion,
                                        mode="raw-task", dataset=self.dataset, poison_type=self.poison_type)
            backdoor_acc, _ = test(self.net_avg, self.device, self.targetted_task_test_loader, test_batch_size=self.test_batch_size, criterion=self.criterion,
        ...
        if self.poison_type == 'ardis':
            ...
            df = pd.concat([df1, df])
        drawacc(main_task_acc, raw_task_acc, backdoor_task_acc, self.dataset)
        ...
        logger.info("\nWrote accuracy results to: {}".format(results_filename))
...
class FixedPoolFederatedLearningTrainer(FederatedLearningTrainer):
    def __init__(self, arguments=None, *args, **kwargs):
        ...
        self.poisoned_emnist_train_loader = arguments['poisoned_emnist_train_loader']
        self.clean_train_loader = arguments['clean_train_loader'] # 用于估计权重
        self.vanilla_emnist_test_loader = arguments['vanilla_emnist_test_loader']
        self.targetted_task_test_loader = arguments['targetted_task_test_loader']

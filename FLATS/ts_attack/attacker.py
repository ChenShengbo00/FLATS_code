...
def get_interval(run_tag, topk):
    ...
    '''
    shaplet_pos = np.loadtxt('../FL_TSadv_master/shapelet_pos/' + run_tag + '_shapelet_pos.txt', usecols=(2, 3))
...
def get_magnitude(run_tag, factor, normalize):
    ...
    '''
    data = load_ucr('../FL_TSadv_master/data/UCR/' + run_tag + '/' + run_tag + '_attack.txt', normalize=normalize)
...
def get_minmax(run_tag, factor, normalize):
    ...
    poisoned_data = np.loadtxt(datadir + args.dataset + '/' + args.dataset + '_attack.txt')
    '''
    data = load_ucr('../FL_TSadv_master/data/UCR/' + run_tag + '/' + run_tag + '_attack.txt',
                    normalize=normalize)
...
class Attacker:
    ...
    def plot_per(self, perturbations, ts, target_class, attack_ts, prior_probs, attack_probs, factor):
        ...
        save_dir = '../FL_TSadv_master/ts_attack/flts_attackfig/' + self.run_tag
    ...
    def plot_per(self, perturbations, ts, target_class, attack_ts, prior_probs, attack_probs, factor):
        ...
        if target_class == -1:
            ...
            plt.title('Targeted(%d): Sample %d, eps_factor=%.3f' %
                      (target_class, 1, factor), fontsize=14)
        plt.legend(loc='upper right', fontsize=8)
        save_dir = '../FL_TSadv_master/ts_attack/flts_attackfig/' + self.run_tag + 'ShapeRandom'
        os.makedirs(save_dir, exist_ok=True)
        plt.savefig(save_dir + '/' + str(factor) + '_' + str(self.top_k)
                    + self.run_tag + '_' + datetime.now().strftime("%Y%m%d_%H%M%S") + '.png')
        # plt.show()
    ...
    def plot_per(self, perturbations, ts, target_class, attack_ts, prior_probs, attack_probs, factor):
        ...
        if target_class == -1:
            ...
            plt.title('Targeted(%d): Sample %d, eps_factor=%.3f' %
                      (target_class, 1, factor), fontsize=14)
        plt.legend(loc='upper right', fontsize=8)
        save_dir = '../FL_TSadv_master/ts_attack/flts_attackfig/' + self.run_tag + 'overAllRandom'
        os.makedirs(save_dir, exist_ok=True)
    ...
    def plot_per(self, ts, target_class, attack_ts, prior_probs, attack_probs, factor):
        ...
        if target_class == -1:
            ...
            plt.title('Targeted(%d): Sample %d, eps_factor=%.3f' %
                      (target_class, 1, factor), fontsize=14)
        plt.legend(loc='upper right', fontsize=8)
        save_dir = '../FL_TSadv_master/ts_attack/flts_attackfig/' + self.run_tag + 'Onepoint'
        os.makedirs(save_dir, exist_ok=True)
